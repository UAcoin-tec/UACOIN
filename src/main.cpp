...
// Copyright (c) 2018 The uacoin developers
...
static const unsigned int timeGenesisBlock = 1588690882;
//static const unsigned int timeGenesisBlock = 1516807985;
static const unsigned int nNonceGenesisBlock = 59287951;
...
const string strMessageMagic = "uacoin Signed Message:\n";

double dHashesPerSec = 0.0;
int64 nHPSTimerStart = 0;

// Settings
int64 nTransactionFee = DEFAULT_BLOCK_MIN_TX_FEE;
                              ...
                              enum GetMinFee_mode mode) const
{
    ...
    if (nMinFee < nBaseFee)
    {
        // std::cout << "[uacoin] CTransaction::GetMinFee() : ";
        
        BOOST_FOREACH(const CTxOut& txout, vout)
        {
            // std::cout << txout.nValue << "(txout.nValue), " << CENT << "(CENT), ";
    ...
    if (nBlockSize != 1 && nNewBlockSize >= MAX_BLOCK_SIZE_GEN/2)
    {
        ...
        // std::cout << "[uacoin] CTransaction::GetMinFee() :" << nMinFee * MAX_BLOCK_SIZE_GEN / (MAX_BLOCK_SIZE_GEN - nNewBlockSize) << " = " 
        //             << nMinFee << " * " << MAX_BLOCK_SIZE_GEN << " / (" << MAX_BLOCK_SIZE_GEN << ")" << nNewBlockSize << std::endl;
...
void WriteDataToConsole(char * data)
{
    // FILE* fp;
...
// static const int64 nPmCoin = 20000000000 * COIN;
// static const int64 nStartCoin = 50 * COIN;
...
static const int64 nRewardforEveryBlock = COIN;                     // default reward is 50 REC
static const int64 nPmCoin = 100000000 * COIN;                     // pre-mined REC 1,920,000,000 (19.2��)
static const int64 nStartCoin = 50 * COIN;

static const int64 nTargetTimespan = 60 * 60 * 24;                  // 86,400 retarget every 1 day
static const int64 nTargetSpacing = 30;                             // 120 -> 30, 30 sec.
static const int64 nInterval = nTargetTimespan / nTargetSpacing;    // 720 -> 2,880

static const int64 BLOCKS_PER_HOUR = 60;                            // 60 blocks per an hour
static const int64 BLOCKS_PER_DAY = BLOCKS_PER_HOUR * 24;           // 60 * 24 (1,440) blocks per a day
static const int64 BLOCKS_PER_YEAR = BLOCKS_PER_DAY * 365;          // 525,600
static const int64 BLOCKS_PER_HALVING_PERIOD = BLOCKS_PER_YEAR * 4; // 2,102,400

// added reward halving rate table for calculate reward reducing rate in every 4 years
double rewardratio[25] = {  20.0f, 18.6f, 17.2f, 15.9f, 14.7f, 
                            13.6f, 12.5f, 11.6f, 10.7f, 9.9f, 
                            9.1f, 8.4f, 7.8f, 7.2f, 6.7f, 
                            6.2f, 5.7f, 5.3f, 4.9f, 4.5f,
                            4.2f, 3.9f, 3.6f, 3.3f, 2.82f       };

static const int64 PREMINING_THRESHOLD = 500;
...
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast, const CBlockHeader *pblock, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax) {
    ...
    if (BlockLastSolved == NULL || BlockLastSolved->nHeight == 0 || (uint64)BlockLastSolved->nHeight < PastBlocksMin || BlockLastSolved->nHeight <= PREMINING_THRESHOLD) {
        return bnProofOfWorkLimit.GetCompact();
    }
...
unsigned int static GetNextWorkRequired_V2(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    //int64 BlocksTargetSpacing = 30;
    static const unsigned int TimeDaySeconds = 60 * 60 * 24;    // 86,400
    int64 PastSecondsMin = TimeDaySeconds * 0.01;               // 864
    int64 PastSecondsMax = TimeDaySeconds * 0.14;               // 12096
    uint64 PastBlocksMin = PastSecondsMin / BlocksTargetSpacing;    // 14.4
    uint64 PastBlocksMax = PastSecondsMax / BlocksTargetSpacing;    // 201.6
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x00000033acb2f88f279d8b06655937261b9fba0f5c6302cf81a7a0385fccef2f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis block
        const char* pszTimestamp = "M-100-1030";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("uacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("uacoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("uacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("uacoin-miner");
	...
	{ 
		while(true) 
		{
			...
			printf("Running uacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
