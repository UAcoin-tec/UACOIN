...
static const char *strMainNetDNSSeed[][3] = {
    //{"Seed A", "175.176.142.110"},
    {"Seed A", "183.111.126.127"},
    {"Seed B", "116.125.141.56"},
    {"Seed C", "222.122.202.218"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. uacoin is probably already running."), addrBind.ToString().c_str());
        else
